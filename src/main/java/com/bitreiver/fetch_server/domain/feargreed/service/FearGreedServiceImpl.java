...
import com.bitreiver.fetch_server.domain.feargreed.dto.FearGreedRedisDto;
...
public class FearGreedServiceImpl implements FearGreedService {
    ...
    @Override
    @Transactional
    public Map<String, Object> fetchRecentDataAndSaveToDb() {
        log.info("공포/탐욕 지수 최근 데이터 DB 증분 저장 시작");
        ...
        String responseBody = alternativeMeWebClient.post()
            ...
            .retrieve()
            .bodyToMono(String.class)
            .doOnError(error -> log.error("fetchRecentDataAndSaveToDb - API 호출 실패: {}", error.getMessage()))
            .block();
        ...
        try {
            ...
            log.error("fetchRecentDataAndSaveToDb - JSON 파싱 실패: {}", e.getMessage(), e);
            throw new CustomException(ErrorCode.FEAR_GREED_API_ERROR, e);
        }
        ...
        // 마지막 인덱스(오늘)은 제외하고 어제까지의 데이터만 저장
        int lastIndex = values.size() - 1;
        for (int i = 0; i < lastIndex; i++) {
            try {
                String dateString = labels.get(i);
                ...
                // 이미 존재하는 날짜는 스킵
                if (fearGreedIndexRepository.existsByDate(date)) {
                    skippedCount++;
                    continue;
                }
                ...
                log.error("fetchRecentDataAndSaveToDb - 데이터 처리 중 오류 발생 (인덱스: {}): {}", i, e.getMessage(), e);
            }
        }
        
        if (!indicesToSave.isEmpty()) {
            fearGreedIndexRepository.saveAll(indicesToSave);
            log.info("공포/탐욕 지수 DB 저장 완료 - 저장: {}개", savedCount);
        }
        
        log.info("공포/탐욕 지수 최근 데이터 DB 증분 저장 완료 - 조회: {}, 저장: {}, 스킵: {}", 
            values.size(), savedCount, skippedCount);
    ...
    @Override
    public void fetchRecentDataAndSaveToRedis() {
        String cookieHeader = "_gid=GA1.2.483577747.1765277934; " +
            "_ga=GA1.1.1708142447.1765028180; " +
            "__gads=ID=3b86a6e7710fdbf4:T=1765028180:RT=1765277933:S=ALNI_MZESCVX-nSSNdEpt9f_xlS0hMZSng; " +
            "__gpi=UID=000011c4160bd281:T=1765028180:RT=1765277933:S=ALNI_MaZdbH7yp-dnf9P7nzbigtFCEE9og; " +
        ...
        String responseBody = alternativeMeWebClient.post()
            ...
            .retrieve()
            .bodyToMono(String.class)
            .doOnError(error -> log.error("fetchRecentDataAndSaveToRedis - API 호출 실패: {}", error.getMessage()))
            .block();
        ...
        try {
            ...
            if (response.getData().getDatasets() != null && !response.getData().getDatasets().isEmpty()) {
                ...
                if (values.size() == labels.size() && !values.isEmpty()) {
                    int todayValue = values.get(values.size() - 1);
                    String todayDateString = labels.get(labels.size() - 1);
                    LocalDate todayDate = LocalDate.parse(todayDateString, DATE_FORMATTER);
                    
                    FearGreedRedisDto todayData = FearGreedRedisDto.builder()
                        .date(todayDate)
                        .dateString(todayDateString)
                        .value(todayValue)
                        .build();
                    
                    redisCacheService.set(REDIS_KEY_TODAY, todayData, 600L);
                    log.info("공포/탐욕 지수 오늘 데이터 Redis 저장 완료 - 지수: {} (날짜: {})", todayValue, todayDate);
                }
            }
        } catch (CustomException e) {
            throw e;
        } catch (Exception e) {
            log.error("fetchRecentDataAndSaveToRedis - 공포/탐욕 지수 오늘 데이터 저장 중 오류 발생: {}", e.getMessage(), e);
            throw new CustomException(ErrorCode.FEAR_GREED_API_ERROR, e);
        }
    }
    
    @Override
    public void fetchYesterdayDataAndSaveToRedis() {
        String cookieHeader = "_gid=GA1.2.483577747.1765277934; " +
            "_ga=GA1.1.1708142447.1765028180; " +
            "__gads=ID=3b86a6e7710fdbf4:T=1765028180:RT=1765277933:S=ALNI_MZESCVX-nSSNdEpt9f_xlS0hMZSng; " +
            "__gpi=UID=000011c4160bd281:T=1765028180:RT=1765277933:S=ALNI_MaZdbH7yp-dnf9P7nzbigtFCEE9og; " +
        ...
        String responseBody = alternativeMeWebClient.post()
            ...
            .retrieve()
            .bodyToMono(String.class)
            .doOnError(error -> log.error("fetchYesterdayDataAndSaveToRedis - API 호출 실패: {}", error.getMessage()))
            .block();
        ...
        try {
            ...
            if (response.getData().getDatasets() != null && !response.getData().getDatasets().isEmpty()) {
                ...
                if (values.size() == labels.size() && values.size() >= 2) {
                    int yesterdayValue = values.get(values.size() - 2);
                    String yesterdayDateString = labels.get(labels.size() - 2);
                    LocalDate yesterdayDate = LocalDate.parse(yesterdayDateString, DATE_FORMATTER);
                    
                    FearGreedRedisDto yesterdayData = FearGreedRedisDto.builder()
                        .date(yesterdayDate)
                    ...
                    List<FearGreedRedisDto> historyList = redisCacheService.get(REDIS_KEY_HISTORY, typeRef)
                        .orElse(new ArrayList<>());
                    
                    boolean exists = historyList.stream()
                        .anyMatch(item -> item.getDate().equals(yesterdayDate));
                    
                    if (!exists) {
                        historyList.add(yesterdayData);
                        redisCacheService.set(REDIS_KEY_HISTORY, historyList, -1L);
                        log.info("공포/탐욕 지수 어제 데이터 Redis 히스토리에 추가 완료 - 지수: {} (날짜: {})", yesterdayValue, yesterdayDate);
                    } else {
                        log.info("공포/탐욕 지수 어제 데이터가 이미 히스토리에 존재합니다 - 날짜: {}", yesterdayDate);
                    }
                }
            }
        } catch (CustomException e) {
            ...
            log.error("fetchYesterdayDataAndSaveToRedis - 공포/탐욕 지수 어제 데이터 저장 중 오류 발생: {}", e.getMessage(), e);
            throw new CustomException(ErrorCode.FEAR_GREED_API_ERROR, e);
        }
    }
    
    @Override
    public void saveAllHistoryToRedis() {
        LocalDate today = LocalDate.now();
        List<FearGreedIndex> allIndices = fearGreedIndexRepository.findByDateLessThan(today);
        
        List<FearGreedRedisDto> historyList = allIndices.stream()
            .map(index -> FearGreedRedisDto.builder()
                .date(index.getDate())
                .dateString(index.getDate().format(DATE_FORMATTER))
        ...
        redisCacheService.set(REDIS_KEY_HISTORY, historyList, -1L);
        log.info("공포/탐욕 지수 전체 히스토리 Redis 저장 완료 - 데이터 수: {}", historyList.size());
    }
}
